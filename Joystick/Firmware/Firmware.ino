...
unsigned long int last_update_t = 0;
...
void drawProgressBar(float p) {
  //Update only every 100ms (slow process)
  if(millis()-last_update_t>100) {
    u8g2.setDrawColor(1);
...
void loop(void) {
  ...
  if( ((!btn0 && btns_last_state[0]) || (!btn1 && btns_last_state[1])) && cnt>40 ) {
